...
/**
 * Extract hostname from a URL string for server mode detection.
 */
...
/**
 * Check whether a hostname belongs to an RFC 1918 private network or loopback.
 */
 ...
 * Priority: explicitMode > env var > hostname-based auto-detection.
 * Auto-detection only flags localhost / private IPs as "self-hosted";
 * all other URLs default to "cloud" to avoid silently dropping auth.
 */
function detectServerMode(baseUrl, explicitMode) {
  if (explicitMode === "self-hosted" || explicitMode === "cloud") return explicitMode;
  ...
  if (isPrivateOrLocalhost(hostname)) return "self-hosted";
...
export async function searchMemory(cfg, payload) {
  ...
  if (cfg.serverMode === "self-hosted") {
    ...
    if (payload.filter) selfHostedPayload.filter = payload.filter;
    if (payload.source) selfHostedPayload.source = payload.source;
    if (payload.knowledgebase_ids) selfHostedPayload.knowledgebase_ids = payload.knowledgebase_ids;
    if (payload.memory_limit_number != null) selfHostedPayload.memory_limit_number = payload.memory_limit_number;
    if (payload.include_preference != null) selfHostedPayload.include_preference = payload.include_preference;
    if (payload.preference_limit_number != null) selfHostedPayload.preference_limit_number = payload.preference_limit_number;
    if (payload.include_tool_memory != null) selfHostedPayload.include_tool_memory = payload.include_tool_memory;
    if (payload.tool_memory_limit_number != null) selfHostedPayload.tool_memory_limit_number = payload.tool_memory_limit_number;
    if (payload.relativity != null) selfHostedPayload.relativity = payload.relativity;
...
export async function addMessage(cfg, payload) {
  ...
  if (cfg.serverMode === "self-hosted") {
    ...
    if (payload.agent_id) selfHostedPayload.agent_id = payload.agent_id;
    if (payload.app_id) selfHostedPayload.app_id = payload.app_id;
    if (payload.source) selfHostedPayload.source = payload.source;
    if (payload.tags) selfHostedPayload.tags = payload.tags;
    if (payload.info) selfHostedPayload.info = payload.info;
    if (payload.allow_public != null) selfHostedPayload.allow_public = payload.allow_public;
    if (payload.allow_knowledgebase_ids) selfHostedPayload.allow_knowledgebase_ids = payload.allow_knowledgebase_ids;
    return callApi(cfg, endpoint, selfHostedPayload);
  }
